...
class MarkdownImportPod extends basev3_1.ImportPod {
    ...
    static async updateLinks({ note, siblingNotes, tree, proc, }) {
        ...
        const links = (0, engine_server_1.selectAll)(engine_server_1.DendronASTTypes.WIKI_LINK, tree);
        links.forEach((link) => {
    ...
    static async updateAssetLinks({ note, tree, assetMap, proc, }) {
        const assetReferences = [
            ...(0, engine_server_1.selectAll)(engine_server_1.DendronASTTypes.IMAGE, tree),
            ...(0, engine_server_1.selectAll)(engine_server_1.DendronASTTypes.LINK, tree),
        ];
        const lines = note.body.split("\n");
    ...
    async plant(opts) {
        ...
        const notesClean = await Promise.all(notes
            ...
            const proc = engine_server_1.MDUtilsV5.procRemarkFull({
                engine,
                fname: note.fname,
                vault: note.vault,
...
class MarkdownPublishPod extends basev3_1.PublishPod {
    ...
    async plant(opts) {
        ...
        let remark = engine_server_1.MDUtilsV5.procRemarkFull({
        ...
        if (wikiLinkToURL && !lodash_1.default.isUndefined(dendronConfig)) {
            remark = remark.use(engine_server_1.RemarkUtils.convertWikiLinkToNoteUrl(note, [], engine, dendronConfig));
        }
        else {
            remark = remark.use(engine_server_1.RemarkUtils.convertLinksFromDotNotation(note, []));
        }
