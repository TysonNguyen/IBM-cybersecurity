...
function string2Note({ content, fname, vault, calculateHash, }) {
    ...
    const contentHash = calculateHash ? (0, common_all_1.genHash)(content) : undefined;
...
async function note2File({ note, vault, wsRoot, }) {
    const { fname } = note;
    const ext = ".md";
    const payload = common_all_1.NoteUtils.serialize(note, { excludeStub: true });
...
function serializeModuleProps(moduleProps) {
...
function schemaModuleProps2File(schemaMProps, vpath, fname) {
    const ext = ".schema.yml";
    return fs_extra_1.default.writeFile(path_1.default.join(vpath, fname + ext), serializeModuleProps(schemaMProps));
}
