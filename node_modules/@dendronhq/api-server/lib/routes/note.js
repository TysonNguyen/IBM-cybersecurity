...
router.post("/links", (0, express_async_handler_1.default)(async (req, res) => {
    const opts = req.body;
    const { ws } = opts;
    const engine = await (0, utils_1.getWSEngine)({ ws });
    const links = await engine.getLinks(opts);
    common_server_1.ExpressUtils.setResponse(res, links);
}));
router.post("/anchors", (0, express_async_handler_1.default)(async (req, res) => {
    const { note } = req.body;
    const anchors = engine_server_1.AnchorUtils.findAnchors({
        note,
    });
    common_server_1.ExpressUtils.setResponse(res, { data: anchors, error: null });
}));
