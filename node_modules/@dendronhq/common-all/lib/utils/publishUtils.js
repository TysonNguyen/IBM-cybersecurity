...
class PublishUtils {
    ...
    static getSEOPropsFromConfig(config) {
        if ((0, intermediateConfigs_1.configIsV4)(config)) {
            const { title, twitter, description: excerpt, image, } = index_1.ConfigUtils.getSite(config);
            return { title, twitter, excerpt, image };
        }
        else {
            const { title, twitter, description: excerpt, image, } = index_1.ConfigUtils.getPublishing(config).seo;
            return { title, twitter, excerpt, image };
        }
    }
