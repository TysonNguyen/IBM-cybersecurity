...
exports.extractNoteChangeEntriesByType = exports.getJournalTitle = exports.Wrap = exports.normalizeUnixPath = exports.cleanName = exports.ConfigUtils = exports.PublishUtils = exports.TagUtils = exports.debounceAsyncUntilComplete = exports.throttleAsyncUntilComplete = exports.mapValues = exports.groupBy = exports.FIFOQueue = exports.NoteFNamesDict = exports.memoize = exports.ListMap = exports.DefaultMap = exports.makeColorTranslucent = exports.randomColor = exports.isFalsy = exports.isNotNull = exports.isNotUndefined = exports.isLineAnchor = exports.isBlockAnchor = exports.isNumeric = exports.getSlugger = exports.DUtils = void 0;
...
class ListMap {
    ...
    delete(key, ...toDelete) {
        ...
        if (values === undefined)
            return;
        lodash_1.default.pull(values, ...toDelete);
        if (values.length === 0) {
...
function memoize({ fn, keyFn = (...args) => args[0].toString(), shouldUpdate = () => false, maxCache = 64, }) {
    ...
    wrapped.cache = new _1.LruCache({ maxItems: maxCache });
    return wrapped;
}
exports.memoize = memoize;
class NoteFNamesDict {
    constructor(initialNotes) {
        this._internalMap = new ListMap();
        if (initialNotes)
            this.addAll(initialNotes);
    }
    get(notes, fname, vault) {
        const keys = this._internalMap.get(cleanName(fname));
        if (keys === undefined)
            return [];
        let matchedNotes = keys.map((key) => notes[key]).filter(isNotUndefined);
        if (vault) {
            matchedNotes = matchedNotes.filter((note) => _1.VaultUtils.isEqualV2(note.vault, vault));
        }
        return matchedNotes;
    }
    ...
    has(note) {
        var _a;
        return !!((_a = this._internalMap
            // there are notes with this fname
            .get(cleanName(note.fname))) === null || _a === void 0 ? void 0 : _a.some((maybeMatch) => maybeMatch === note.id));
    }
    add(note) {
        if (this.has(note))
...
exports.NoteFNamesDict = NoteFNamesDict;
class FIFOQueue {
    constructor(init) {
        this._internalQueue = [];
        if (init)
...
class PublishUtils {
    static getSEOPropsFromConfig(config) {
        if ((0, intermediateConfigs_1.configIsV4)(config)) {
            const { title, twitter, description: excerpt, image, } = ConfigUtils.getSite(config);
            return { title, twitter, excerpt, image };
        }
        else {
            const { title, twitter, description: excerpt, image, } = ConfigUtils.getPublishing(config).seo;
            return { title, twitter, excerpt, image };
        }
    }
...
class ConfigUtils {
    ...
    static genDefaultV4Config() {
        ...
        return {
            version: 4,
            ...common,
            commands: (0, intermediateConfigs_1.genDefaultCommandConfig)(),
            workspace: (0, intermediateConfigs_1.genDefaultWorkspaceConfig)(),
            preview: (0, intermediateConfigs_1.genDefaultPreviewConfig)(),
        };
    }
     ...
     */
    static getSite(config) {
        const v4DefaultConfig = ConfigUtils.genDefaultV4Config();
        const configWithDefaults = lodash_1.default.defaultsDeep(config, v4DefaultConfig);
    ...
    static getPublishingConfig(config) {
        return (0, intermediateConfigs_1.configIsV4)(config)
    ...
    static getEnableFMTitle(config, shouldApplyPublishRules) {
        const publishRule = (0, intermediateConfigs_1.configIsV4)(config)
    ...
    static getEnableMermaid(config, shouldApplyPublishRules) {
        const publishRule = (0, intermediateConfigs_1.configIsV4)(config)
    ...
    static getGithubConfig(config) {
        if ((0, intermediateConfigs_1.configIsV4)(config)) {
            const { gh_edit_link, gh_edit_link_text, gh_edit_repository, gh_edit_branch, gh_edit_view_mode, githubCname, } = ConfigUtils.getSite(config);
            return {
                cname: githubCname,
    ...
    static getUseContainers(config) {
        var _a;
        return (_a = ConfigUtils.getSite(config)) === null || _a === void 0 ? void 0 : _a.useContainers;
    }
    static getEnableRandomlyColoredTags(config) {
        var _a;
        return (0, intermediateConfigs_1.configIsV4)(config)
            ? !((_a = ConfigUtils.getSite(config)) === null || _a === void 0 ? void 0 : _a.noRandomlyColoredTags)
    ...
    static getEnableFrontmatterTags(opts) {
        ...
        const publishRule = (0, intermediateConfigs_1.configIsV4)(config)
            ? (_a = ConfigUtils.getSite(config)) === null || _a === void 0 ? void 0 : _a.showFrontMatterTags
    ...
    static getEnableHashesForFMTags(opts) {
        ...
        const { config, shouldApplyPublishRules } = opts;
        const publishRule = (0, intermediateConfigs_1.configIsV4)(config)
            ? (_a = ConfigUtils.getSite(config)) === null || _a === void 0 ? void 0 : _a.useHashesForFMTags
    ...
    static getEnablePrettlyLinks(config) {
        var _a;
        return (0, intermediateConfigs_1.configIsV4)(config)
            ? (_a = ConfigUtils.getSite(config)) === null || _a === void 0 ? void 0 : _a.usePrettyLinks
            : ConfigUtils.getPublishing(config).enablePrettyLinks;
    }
    static getGATracking(config) {
        var _a, _b;
        return (0, intermediateConfigs_1.configIsV4)(config)
            ? (_a = ConfigUtils.getSite(config)) === null || _a === void 0 ? void 0 : _a.ga_tracking
            : (_b = ConfigUtils.getPublishing(config).ga) === null || _b === void 0 ? void 0 : _b.tracking;
    }
    static getSiteLastModified(config) {
        var _a;
        return (0, intermediateConfigs_1.configIsV4)(config)
            ? (_a = ConfigUtils.getSite(config)) === null || _a === void 0 ? void 0 : _a.siteLastModified
    ...
    static getEnablePrettyRefs(config, opts) {
        ...
        if (override !== undefined)
            return override;
        const publishRule = (0, intermediateConfigs_1.configIsV4)(config)
            ? (_d = ConfigUtils.getSite(config)) === null || _d === void 0 ? void 0 : _d.usePrettyRefs
    ...
    static setSiteProp(config, key, value) {
        const path = `site.${key}`;
        lodash_1.default.set(config, path, value);
    }
    ...
    static unsetSiteProp(config, key) {
        const path = `site.${key}`;
    ...
    static setDuplicateNoteBehavior(config, value) {
        if ((0, intermediateConfigs_1.configIsV4)(config)) {
            ConfigUtils.setSiteProp(config, "duplicateNoteBehavior", value);
        }
        else {
    ...
    static updateVault(config, vaultToUpdate, updateCb) {
        ConfigUtils.setVaults(config, ConfigUtils.getVaults(config).map((configVault) => {
            if (!_1.VaultUtils.isEqualV2(vaultToUpdate, configVault))
                return configVault;
...
function normalizeUnixPath(fsPath) {
    return path_1.default.posix.format(path_1.default.parse(fsPath));
}
...
function getJournalTitle(noteName, dateFormat) {
    ...
    while (title.length > 0) {
        const attemptedParse = _1.DateTime.fromFormat(title.join("."), dateFormat);
