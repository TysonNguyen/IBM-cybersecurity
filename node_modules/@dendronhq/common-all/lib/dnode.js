...
class DNodeUtils {
    ...
    static findClosestParent(fpath, nodes, notesByFnameDict, opts) {
        const { vault } = opts;
        ...
        if ((maybeNode && !(opts === null || opts === void 0 ? void 0 : opts.noStubs)) ||
            ...
            return DNodeUtils.findClosestParent(dirname, nodes, notesByFnameDict, opts);
        }
    }
    /**
    ...
    static cleanFname(fname) {
        if (fname.indexOf(" ") >= 0) {
            fname = lodash_1.default.replace(fname, /\s/g, "-");
        }
        return lodash_1.default.kebabCase(fname);
    }
}
...
class NoteUtils {
    ...
    static addOrUpdateParentsWithDict(opts) {
        ...
        if (!parent) {
            const ancestor = DNodeUtils.findClosestParent(note.fname, notesDict, notesByFnameDict, {
                vault: note.vault,
                wsRoot,
            });
    ...
    static addOrUpdateParents(opts) {
        ...
        return NoteUtils.addOrUpdateParentsWithDict({
            note,
            notesDict,
            notesByFnameDict: new utils_1.NoteFNamesDict(lodash_1.default.values(notesDict)),
            createStubs,
            wsRoot,
        });
    }
    ...
    static createWithSchema({ noteOpts, engine, }) {
        ...
        if (maybeMatch) {
            ...
            if (maybeTemplate) {
                ...
                const tempNote = tempInSameVault ||
     ...
     */
    static getNoteLocalConfig(note) {
        if (note.config) {
            return note.config;
        }
        return {};
...
class SchemaUtils {
    ...
    static applyTemplate(opts) {
        ...
        if (templateNote.type === "note") {
            const tempNoteProps = lodash_1.default.pick(templateNote, this.TEMPLATE_COPY_PROPS);
            ...
            const currentDate = time_1.Time.now();
            note.body = note.body.replace(/<%=\s*CURRENT_YEAR\s*%>/g, currentDate.toFormat("yyyy"));
            note.body = note.body.replace(/<%=\s*CURRENT_MONTH\s*%>/g, currentDate.toFormat("LL"));
            note.body = note.body.replace(/<%=\s*CURRENT_WEEK\s*%>/g, currentDate.toFormat("WW"));
            note.body = note.body.replace(/<%=\s*CURRENT_DAY\s*%>/g, currentDate.toFormat("dd"));
            note.body = note.body.replace(/<%=\s*CURRENT_HOUR\s*%>/g, currentDate.toFormat("HH"));
            note.body = note.body.replace(/<%=\s*CURRENT_MINUTE\s*%>/g, currentDate.toFormat("mm"));
            note.body = note.body.replace(/<%=\s*CURRENT_SECOND\s*%>/g, currentDate.toFormat("ss"));
            return true;
        }
        return false;
    }
