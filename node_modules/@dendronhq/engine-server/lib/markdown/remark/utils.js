...
const getLinks = ({ ast, note, filter, }) => {
    ...
    const { logger, dispose } = (0, common_server_1.createDisposableLogger)("LinkUtils.getLinks");
    logger.info({
        ctx: "getLinks",
        dlinksLength: dlinks.length,
        noteRefsLength: noteRefs.length,
        wikiLinksLength: wikiLinks.length,
        filterLocFname: (_a = filter === null || filter === void 0 ? void 0 : filter.loc) === null || _a === void 0 ? void 0 : _a.fname,
    });
    dispose();
    return dlinks;
};
const getLinkCandidates = ({ ast, note, engine, }) => {
    const textNodes = [];
    ...
    lodash_1.default.map(textNodes, (textNode) => {
        ...
        value.split(/\s+/).forEach((word) => {
            const possibleCandidates = common_all_1.NoteUtils.getNotesByFnameFromEngine({
                fname: word,
                engine,
                vault: note.vault,
...
class LinkUtils {
    ...
    static parseDendronURI(linkString) {
        if (linkString.startsWith(common_all_1.CONSTANTS.DENDRON_DELIMETER)) {
            const [vaultName, link] = linkString
                .split(common_all_1.CONSTANTS.DENDRON_DELIMETER)[1]
                .split("/");
            return {
                vaultName,
                link,
    ...
    static getNotesFromWikiLinks(opts) {
        ...
        wikiLinks.forEach((wikiLink) => {
            ...
            if (vault) {
                ...
                const notesWithSameFname = common_all_1.NoteUtils.getNotesByFnameFromEngine({
                    fname,
                    engine,
                });
    ...
    static findLinkCandidates({ note, engine, }) {
        const content = note.body;
        ...
        const linkCandidates = getLinkCandidates({
            ast: tree,
            note,
            engine,
        });
    ...
    static findAnchors(opts) {
        ...
        try {
            ...
            const error = common_all_1.DendronError.createFromStatus({
                ...
                payload: { note: common_all_1.NoteUtils.toLogObj(opts.note) },
                innerError: err,
            });
            const { logger, dispose } = (0, common_server_1.createDisposableLogger)("AnchorUtils");
            logger.error(error);
            dispose();
            return {};
        }
...
class RemarkUtils {
    ...
    static findAnchors(content) {
        const parser = utilsv5_1.MDUtilsV5.procRehypeParse({
            mode: utilsv5_1.ProcMode.NO_DATA,
        });
        ...
        return function () {
            return (tree, _vfile) => {
                ...
                wikiLinks.forEach((linkNode) => {
                    ...
                    const existingNote = common_all_1.NoteUtils.getNoteFromMultiVault({
                    ...
                    if (existingNote) {
                        const publishingConfig = common_all_1.ConfigUtils.getPublishingConfig(dendronConfig);
                        const urlRoot = publishingConfig.siteUrl || "";
                        const { vault } = existingNote;
                        linkNode.value = workspace_1.WorkspaceUtils.getNoteUrl({
     ...
     */
    static extractHeaderBlock(tree, targetHeader) {
        let headerFound = false;
        let foundHeaderIndex;
        let nextHeaderIndex;
        (0, unist_util_visit_1.default)(tree, (node, index) => {
            if (nextHeaderIndex) {
                return;
            }
            // @ts-ignore
            ...
            else if (node.type === types_1.DendronASTTypes.HEADING) {
                if (foundHeaderIndex) {
                    if (depth <= targetHeader.depth)
        ...
        const nodesToExtract = nextHeaderIndex
            ? tree.children.splice(foundHeaderIndex, nextHeaderIndex - foundHeaderIndex)
     ...
     */
    static async extractBlocks({ note, engine, }) {
    ...
    static extractFMTags(body) {
        let parsed;
        const noteAST = utilsv5_1.MDUtilsV5.procRemarkParse({ mode: utilsv5_1.ProcMode.NO_DATA }, {}).parse(body);
        (0, unist_util_visit_1.default)(noteAST, [types_1.DendronASTTypes.FRONTMATTER], (frontmatter) => {
            parsed = (0, common_server_1.parseFrontmatter)(frontmatter);
            return false; // stop traversing, there is only one frontmatter
        });
        if (parsed) {
            return (0, common_server_1.getFrontmatterTags)(parsed);
        }
        else {
            return [];
        }
