...
const tryGetNotes = ({ fname, vname, vaults, engine, }) => {
    const maybeVault = vname
        ...
        : undefined;
    const maybeNotes = common_all_1.NoteUtils.getNotesByFnameFromEngine({
        fname,
        engine,
        vault: maybeVault,
    });
...
function attachCompiler(proc, opts) {
    ...
    if (visitors) {
        visitors.refLinkV2 = function refLinkV2(node) {
...
function convertNoteRef(opts) {
    ...
    const engine = procData.engine || __1.MDUtilsV4.getEngineFromProc(proc).engine;
    let { vault } = procData;
    const shouldApplyPublishRules = utilsv5_1.MDUtilsV5.shouldApplyPublishingRules(proc) ||
        __1.MDUtilsV4.getDendronData(proc).shouldApplyPublishRules;
    ...
    const containingNote = common_all_1.NoteUtils.getNoteByFnameFromEngine({
        fname,
        vault,
        engine,
    });
    if (prettyRefs === undefined) {
        prettyRefs = common_all_1.ConfigUtils.getEnablePrettyRefs(config, {
            note: containingNote,
            shouldApplyPublishRules,
        });
    }
    if ((containingNote === null || containingNote === void 0 ? void 0 : containingNote.custom.usePrettyRefs) !== undefined &&
        lodash_1.default.isBoolean(containingNote.custom.usePrettyRefs)) {
    ...
    const out = noteRefs.map((ref) => {
        ...
        try {
            const note = (0, common_server_1.file2Note)(npath, vault);
...
function convertNoteRefASTV2(opts) {
    ...
    function processRef(ref, note, fname) {
        try {
            ...
            if (prettyRefs) {
                ...
                return (0, mdast_builder_1.paragraph)(data);
            }
        }
        catch (err) {
    ...
    let prettyRefs = common_all_1.ConfigUtils.getEnablePrettyRefs(config, {
        shouldApplyPublishRules,
        note: containingNote,
    });
    if (prettyRefs &&
    ...
    if (link.from.fname.endsWith("*")) {
        ...
        const processedRefs = noteRefs.map((ref) => {
            ...
            let note;
            try {
                note = (0, common_server_1.file2Note)(npath, vault);
            }
            catch (err) {
                const msg = `error reading file, ${npath}`;
        ...
        if (resp.error) {
            return {
                error: undefined,
                data: [__1.MdastUtils.genMDErrorMsg(resp.error.message)],
            };
        }
        // multiple results
...
function convertNoteRefHelperAST(opts) {
    ...
    const engine = utilsv5_1.MDUtilsV5.getProcData(proc).engine ||
        __1.MDUtilsV4.getEngineFromProc(proc).engine;
    ...
    noteRefProc = noteRefProc.data("fm", utilsv5_1.MDUtilsV5.getFM({ note, wsRoot }));
    ...
    const { start, end, data, error } = prepareNoteRefIndices({
        ...
        makeErrorData: (anchorName, anchorType) => {
            return __1.MdastUtils.genMDMsg(`${anchorType} anchor ${anchorName} not found`);
        },
    });
    if (data)
        return { data, error };
...
function convertNoteRefHelper(opts) {
    ...
    utilsv5_1.MDUtilsV5.setProcData(noteRefProc, { fname: link.from.fname });
    utilsv5_1.MDUtilsV5.setNoteRefLvl(noteRefProc, refLvl);
...
function findAnchor({ nodes, match, }) {
    if ((0, common_all_1.isBlockAnchor)(match)) {
        ...
        return findHeader({ nodes, match, slugger: (0, common_all_1.getSlugger)() });
    }
}
function findHeader({ nodes, match, slugger, }) {
    const foundIndex = __1.MdastUtils.findIndex(nodes, (node, idx) => {
        if (idx === 0 && match === "*") {
            return false;
        }
        return __1.MdastUtils.matchHeading(node, match, { slugger });
    });
    if (foundIndex < 0)
        return null;
