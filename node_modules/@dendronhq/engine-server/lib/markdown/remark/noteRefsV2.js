...
function gatherNoteRefs({ link, engine, vault, }) {
    let noteRefs = [];
...
function convertNoteRefASTV2(opts) {
    ...
    const noteRefs = gatherNoteRefs({ link, engine, vault });
    if (link.from.fname.endsWith("*")) {
...
function prepareNoteRefIndices({ anchorStart, anchorEnd, bodyAST, makeErrorData, enableSmartRef, }) {
    var _a;
...
function convertNoteRefHelperAST(opts) {
    ...
    const { engine, config } = utilsv5_1.MDUtilsV5.getProcData(proc);
    ...
    const { start, end, data, error } = prepareNoteRefIndices({
        anchorStart,
        enableSmartRef: common_all_1.ConfigUtils.getWorkspace(config).enableSmartRefs,
        anchorEnd,
        bodyAST,
