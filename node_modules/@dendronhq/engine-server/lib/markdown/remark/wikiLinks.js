...
const parseWikiLink = (linkMatch) => {
    ...
    return utils_2.LinkUtils.parseLinkV2({ linkString: linkMatch });
};
...
function attachCompiler(proc, opts) {
    ...
    if (visitors) {
        visitors.wikiLink = function (node) {
            ...
            const { dest, vault } = utilsv5_1.MDUtilsV5.getProcData(proc);
            ...
            if (copts.useId && dest === types_1.DendronASTDest.HTML) {
                ...
                const { error, note } = (0, utils_2.getNoteOrError)(notes, value);
                if (error) {
                    (0, utils_2.addError)(proc, error);
            ...
            switch (dest) {
                ...
                case types_1.DendronASTDest.MD_ENHANCED_PREVIEW: {
                    ...
                    if (data.vaultName) {
                        ...
                        const cpath = (0, common_server_1.vault2Path)({
                            wsRoot: engine.wsRoot,
                            vault: utils_1.MDUtilsV4.getVault(proc),
                        });
                        const npath = (0, common_server_1.vault2Path)({
                            vault: vaultByName,
                            wsRoot: engine.wsRoot,
                        });
...
function attachParser(proc) {
    ...
    function parseLink(linkMatch) {
        const pOpts = utilsv5_1.MDUtilsV5.getProcOpts(proc);
        linkMatch = common_all_1.NoteUtils.normalizeFname(linkMatch);
        const out = utils_2.LinkUtils.parseLinkV2({ linkString: linkMatch });
