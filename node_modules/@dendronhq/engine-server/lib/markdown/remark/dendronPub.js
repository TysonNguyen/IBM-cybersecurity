...
function replacedUnrenderedRefWithConvertedData(data, children) {
    if (children.length > 1) {
        const idx = lodash_1.default.findIndex(children, utils_2.RemarkUtils.isNoteRefV2);
...
function plugin(opts) {
    const proc = this;
    let { overrides, vault } = utils_1.MDUtilsV4.getDendronData(proc);
    ...
    function transformer(tree, _file) {
        const root = tree;
        const { error: engineError, engine } = utils_1.MDUtilsV4.getEngineFromProc(proc);
        ...
        if (mode !== utilsv5_1.ProcMode.IMPORT && !insideNoteRef && root.children) {
            ...
            if (fname === "403") {
                note = site_1.SiteUtils.create403StaticNote({ engine });
            }
            else {
        ...
        (0, unist_util_visit_parents_1.default)(tree, (node, ancestors) => {
            ...
            if (lodash_1.default.isUndefined(parent) || !utils_2.RemarkUtils.isParent(parent))
                return; // root node
            if (node.type === types_1.DendronASTTypes.HASHTAG) {
                ...
                if (!ancestors.some((node) => utils_2.RemarkUtils.isLink(node))) {
                    node = (0, utils_2.hashTag2WikiLinkNoteV4)(hashtag);
                }
                else {
            ...
            if (node.type === types_1.DendronASTTypes.USERTAG) {
                ...
                // but not if they are inside of a link, otherwise they break link rendering.
                if (!ancestors.some((node) => utils_2.RemarkUtils.isLink(node))) {
                    node = (0, utils_2.userTag2WikiLinkNoteV4)(userTag);
                }
                else {
            ...
            if (node.type === types_1.DendronASTTypes.WIKI_LINK &&
                ...
                if (engineError) {
                    (0, utils_2.addError)(proc, engineError);
                }
                let error;
                let note;
                ...
                if (!note && (opts === null || opts === void 0 ? void 0 : opts.transformNoPublish)) {
                    ...
                    (0, utils_2.addError)(proc, new common_all_1.DendronError({
                        message: "no note",
                        code,
                ...
                const href = site_1.SiteUtils.getSiteUrlPathForNote({
            ...
            if (node.type === types_1.DendronASTTypes.BLOCK_ANCHOR) {
                ...
                if (utils_2.RemarkUtils.isParagraph(parent) &&
                    parent.children.length === 1 &&
                    (0, common_all_1.isNotUndefined)(grandParent) &&
                    utils_2.RemarkUtils.isRoot(grandParent)) {
                ...
                else if (utils_2.RemarkUtils.isTableRow(grandParent)) {
                    ...
                    if ((0, common_all_1.isNotUndefined)(greatGrandParent) &&
                        utils_2.RemarkUtils.isTable(greatGrandParent)) {
