...
exports.PublishUtils = exports.MdastUtils = exports.MDUtilsV4 = exports.renderFromNote = void 0;
...
class MDUtilsV4 {
    ...
    static procFull(opts) {
        ...
        if (lodash_1.default.isUndefined(usePrettyRefs)) {
            usePrettyRefs = common_all_1.ConfigUtils.getEnablePrettyRefs(config, {
                shouldApplyPublishRules,
                note,
            });
        }
        const insertTitle = common_all_1.ConfigUtils.getEnableFMTitle(config, shouldApplyPublishRules);
        proc = proc
            .data("dendron", {
            dest,
            vault,
        ...
        if (lodash_1.default.isUndefined((_a = opts.wikiLinksOpts) === null || _a === void 0 ? void 0 : _a.convertLinks) ||
            ((_b = opts.wikiLinksOpts) === null || _b === void 0 ? void 0 : _b.convertLinks)) {
            proc = proc.use(wikiLinks_1.wikiLinks, opts.wikiLinksOpts).use(backlinks_1.backlinks);
        }
        if (((_c = opts.mathOpts) === null || _c === void 0 ? void 0 : _c.katex) ||
            common_all_1.ConfigUtils.getEnableKatex(config, shouldApplyPublishRules)) {
            proc = proc.use(remark_math_1.default);
        }
        if (opts.mermaid ||
            common_all_1.ConfigUtils.getEnableMermaid(config, shouldApplyPublishRules)) {
    ...
    static procRehype(opts) {
        ...
        let _proc = proc || (0, unified_1.default)().use(remark_parse_1.default, { gfm: true });
        mdPlugins.forEach((p) => {
            _proc = _proc.use(p);
        });
        _proc = _proc
    ...
    static procHTML(procOpts) {
        ...
        let proc = MDUtilsV4.procFull({
            ...
            publishOpts: {
                assetsPrefix: (0, common_all_1.getStage)() === "prod" ? publishingConfig.assetsPrefix : undefined,
                insertTitle: common_all_1.ConfigUtils.getProp(config, "useFMTitle"),
                transformNoPublish: true,
            },
            mathOpts: { katex: true },
            mermaid: common_all_1.ConfigUtils.getProp(config, "mermaid"),
            config,
        });
        proc = proc.use(publishSite_1.publishSite, { noteIndex });
        const useContainers = common_all_1.ConfigUtils.getUseContainers(config);
        if (useContainers) {
...
class MdastUtils {
    ...
    static findHeader({ nodes, match, slugger, }) {
        ...
        const cMatchText = lodash_1.default.isString(match)
            ? match
            : (0, utils_1.normalizev2)(toString(match), (0, common_all_1.getSlugger)());
        let foundNode;
    ...
    static async visitAsync(tree, test, visitor) {
        ...
        while (visitQueue.length > 0) {
            ...
            if (remark_2.RemarkUtils.isParent(node))
