...
const decorations_1 = require("./markdown/decorations");
...
class DendronEngineV2 {
    ...
    static create({ wsRoot, logger }) {
        ...
        const config = config_1.DConfig.readConfigAndApplyLocalOverrideSync(wsRoot);
    ...
    async renderNote({ id, note, flavor, dest, }) {
        ...
        try {
            data = await this._renderNote({
                note,
                flavor: flavor || markdown_1.ProcFlavor.PREVIEW,
    ...
    async _renderNote({ note, flavor, dest, }) {
        ...
        if (dest === common_all_1.DendronASTDest.HTML) {
            proc = markdown_1.MDUtilsV5.procRehypeFull({
                engine: this,
                fname: note.fname,
                vault: note.vault,
                config: this.config,
            ...
            proc = markdown_1.MDUtilsV5.procRemarkFull({
                engine: this,
                fname: note.fname,
                vault: note.vault,
                dest,
    ...
    async refreshNotesV2(notes) {
        await Promise.all(notes.map(async (ent) => {
            ...
            if (ent.status === "delete") {
                ...
                const note = await _1.EngineUtils.refreshNoteLinksAndAnchors({
                    note: ent.note,
                    engine: this,
                });
                this.notes[id] = note;
    ...
    async getNoteBlocks(opts) {
        ...
        try {
            ...
            const blocks = await utils_1.RemarkUtils.extractBlocks({
                note,
                engine: this,
            });
    ...
    async getDecorations(opts) {
        ...
        try {
            ...
            const { allDecorations: decorations, allDiagnostics: diagnostics, allErrors: errors, } = await (0, decorations_1.runAllDecorators)({ ...opts, note, engine: this });
            let error = null;
