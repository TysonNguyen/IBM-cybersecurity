...
const utils_2 = require("../../markdown/remark/utils");
/**
...
class NoteParser extends parseBase_1.ParserBase {
    ...
    async parseFiles(allPaths, vault) {
        ...
        const notesByFname = new common_all_1.NoteFNamesDict();
        const notesById = {};
        ...
        if (lodash_1.default.size(notesById) > 0) {
    ...
    parseNoteProps(opts) {
        ...
        if (cleanOpts.addParent) {
            const stubs = common_all_1.NoteUtils.addOrUpdateParentsWithDict({
                note: noteProps,
                notesDict,
    ...
    file2NoteWithCache({ fpath, vault, toLowercase, errors, }) {
        ...
        const { name } = path_1.default.parse(fpath);
        const sig = (0, common_server_1.genHash)(content);
        ...
        note = (0, common_server_1.string2Note)({ content, fname, vault });
    ...
    updateLinksAndAnchors(note) {
        const ctx = "noteParser:updateLinksAndAnchors";
        // Skip finding links/anchors if note is too long
        if (note.body.length >=
            (this.maxNoteLength || common_all_1.CONSTANTS.DENDRON_DEFAULT_MAX_NOTE_LENGTH)) {
            this.logger.info({
                ctx,
            ...
            throw new common_all_1.DendronError({
                message: `Note "${note.fname}" in vault "${common_all_1.VaultUtils.getName(note.vault)}" is longer than ${this.maxNoteLength || common_all_1.CONSTANTS.DENDRON_DEFAULT_MAX_NOTE_LENGTH} characters, some features like backlinks may not work correctly for it. ` +
                    `You may increase "maxNoteLength" in "dendron.yml" to override this warning.`,
                severity: common_all_1.ERROR_SEVERITY.MINOR,
            });
        }
        try {
            const links = utils_2.LinkUtils.findLinks({
                note,
                engine: this.engine,
            });
            note.links = links;
        ...
        try {
            const anchors = utils_2.AnchorUtils.findAnchors({
                note,
            });
