...
class WorkspaceService {
    ...
    get config() {
        ...
        const { error, data } = config_1.DConfig.readConfigAndApplyLocalOverrideSync(this.wsRoot);
        if (error)
    ...
    async migrateVaultToSelfContained({ vault }) {
        ...
        // Move all note files
        const noteFiles = await (0, common_server_1.getAllFiles)({ root: oldFolder, include: ["*.md"] });
        if (!noteFiles.data) {
        ...
        const config = config_1.DConfig.getRaw(this.wsRoot);
        ...
        await config_1.DConfig.createBackup(this.wsRoot, backupInfix);
        await config_1.DConfig.writeConfig({ wsRoot: this.wsRoot, config });
        ...
        if (!(await fs_extra_1.default.pathExists(path_1.default.join(newFolder, common_all_1.CONSTANTS.DENDRON_CONFIG_FILE)))) {
            ...
            await config_1.DConfig.createBackup(newFolder, backupInfix);
            const config = config_1.DConfig.getOrCreate(newFolder);
            common_all_1.ConfigUtils.setVaults(config, [vaultConfig]);
            await config_1.DConfig.writeConfig({ wsRoot: newFolder, config });
        }
        // Create or update the workspace file (dendron.code-workspace) inside the vault
        if (!(await fs_extra_1.default.pathExists(path_1.default.join(newFolder, common_all_1.CONSTANTS.DENDRON_WS_NAME)))) {
