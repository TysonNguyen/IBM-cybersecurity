...
class DoctorService {
    ...
    findBrokenLinks(note, noteDicts, engine) {
        ...
        const out = lodash_1.default.filter(links, (link) => {
            ...
            const noteExists = common_all_1.NoteDictsUtils.findByFname(link.to.fname, noteDicts, hasVaultPrefix ? vault : note.vault)[0];
            if (hasVaultPrefix) {
    ...
    getBrokenLinkDestinations(notes, engine) {
        ...
        lodash_1.default.forEach(notes, (note) => {
            ...
            const brokenLinks = this.findBrokenLinks(note, { notesById, notesByFname }, engine);
    ...
    async executeDoctorActions(opts) {
        ...
        if (lodash_1.default.isUndefined(candidates)) {
            notes =
                (_a = (query
                    ...
                    : lodash_1.default.values(engine.notes))) !== null && _a !== void 0 ? _a : [];
        }
        else {
            notes = candidates;
        }
        ...
        switch (action) {
            case DoctorActionsEnum.REMOVE_DEPRECATED_CONFIGS: {
                ...
                const rawConfig = config_1.DConfig.getRaw(wsRoot);
                ...
                if (pathsToDelete.length > 0) {
                    ...
                    await config_1.DConfig.writeConfig({ wsRoot, config: configDeepCopy });
                    return {
                        exit: true,
                        resp: {
                            backupPath,
            ...
            case DoctorActionsEnum.ADD_MISSING_DEFAULT_CONFIGS: {
                ...
                if (detectOut) {
                    ...
                    if (needsBackfill) {
                        ...
                        // write config
                        await config_1.DConfig.writeConfig({ wsRoot, config: backfilledConfig });
                        return {
                            exit: true,
                            resp: {
                                backupPath,
            ...
            case DoctorActionsEnum.H1_TO_TITLE: {
                doctorAction = async (note) => {
                    const changes = [];
                    const proc = __1.MDUtilsV5._procRemark({
                        mode: markdown_1.ProcMode.IMPORT,
            ...
            case DoctorActionsEnum.FIX_AIRTABLE_METADATA: {
                ...
                notes = Object.values(notes).filter((value) => value.fname.startsWith(selectedHierarchy) &&
                    value.stub !== true &&
    ...
    async createBackup(wsRoot, backupInfix) {
        try {
            const path = await config_1.DConfig.createBackup(wsRoot, backupInfix);
            return path;
        }
        catch (error) {
